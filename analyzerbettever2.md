...
// Load html2canvas for full-report image generation
useEffect(() => {
...
if (!analysis) return;
    ...
    a.href = url;
    a.download = `Medical_Report_${analysis.overview.date.replace(/\//g, '-')}.txt`;
...
if (!analysis || !window.html2canvas) return;
    ...
    setLoading(true);
    try {
      // Small timeout to ensure DOM is ready and styles are stable
      await new Promise(r => setTimeout(r, 200));

      const canvas = await window.html2canvas(reportRef.current, {
        scale: 2, // High DPI for clear text
        useCORS: true,
        backgroundColor: '#f8fafc', // Matches bg-slate-50
        logging: false,
        onclone: (clonedDoc) => {
          // Ensure glossary is visible in the capture if it's rendered
          const glossary = clonedDoc.getElementById('glossary-section');
      ...
      link.download = `MedScan_Report_${analysis.overview.date.replace(/\//g, '-')}.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
...
<button className="text-indigo-600 hover:text-indigo-700 transition-colors">Help</button>
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-700">
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 print:hidden px-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-slate-800 text-sm">Analysis Ready</h3>
                    <p className="text-[10px] text-slate-500">Ready to save or export findings</p>
                  </div>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <button
                    onClick={scrollToGlossary}
                    className="bg-white hover:bg-slate-50 border border-slate-200 px-3 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors shadow-sm"
                  >
                    <BookOpen className="w-4 h-4 text-indigo-500" /> Glossary
                  </button>
                  <button
                    onClick={downloadReportText}
                    className="bg-white hover:bg-slate-50 border border-slate-200 px-3 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-colors shadow-sm"
                  >
                    <Download className="w-4 h-4 text-slate-400" /> Export Text
                  </button>
                  <button
                    onClick={saveReportAsImage}
                    className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all shadow-md shadow-indigo-100"
                  >
                    <ImageIcon className="w-4 h-4" /> Save Full Report (Image)
                  </button>
                </div>
              </div>

              {/* THE REPORT CONTAINER (Captures this for the image) */}
              <div ref={reportRef} id="full-report-content" className="space-y-6 p-1 sm:p-4 bg-slate-50 rounded-2xl">
                ...
                <div className="bg-indigo-600 p-6 rounded-2xl shadow-sm text-white">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    ...
                    <div className="hidden sm:block">
                      <div className="bg-white/10 px-3 py-1 rounded text-[10px] font-bold tracking-widest uppercase">
                        MedScan Verified
                      </div>
                    </div>
                  </div>
                          ...
                          {analysis.biomarkers.map((item, idx) => (
                            <tr key={idx} className="border-b border-slate-50 last:border-0">
                              <td className="py-4 pr-4">
                                ...
                                <div className="text-[10px] text-slate-400 font-medium">Verified Extraction</div>
                              </td>
                ...
                {/* Full Glossary (Included in image capture) */}
                <div ref={glossaryRef} id="glossary-section" className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                    ...
                    {analysis.biomarkers.map((item, idx) => (
                      <div key={idx} className="bg-slate-50/50 p-4 rounded-xl border border-slate-100">
                        ...
                        <p className="text-xs text-slate-600 leading-relaxed">
                          {item.note || "Refer to clinician for detailed physiology regarding this specific value."}
                        </p>
                      </div>
                    ))}
                  </div>
                ...
                <div className="text-center py-4 border-t border-slate-100">
                  <p className="text-[10px] text-slate-400 font-medium">
                    {analysis.disclaimer} • Generated by MedScan AI Platform • {new Date().toLocaleDateString()}
                  </p>
                </div>
              </div>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        @media print {
          ...
          nav, .print\\:hidden, .lg\\:col-span-4 { display: none !important; }
          #full-report-content { background-color: white !important; padding: 0 !important; }
