...
const refId = `MF-${Math.random().toString(36).substr(2, 9).toUpperCase()}`;
...
DATE: ${date}
TIME: ${time}
CASE REFERENCE ID: ${refId}

${line}
PATIENT INFORMATION
${line}
Patient Name : ${reportData.patientName}
Patient Age : ${reportData.patientAge} Years
Clinical Status : Assessment Completed
...
${line}
${reportData.nextSteps || "Schedule a physical examination with your primary care provider."}

${separator}
                    IMPORTANT NOTICE
${separator}
This report was generated by the MediFlow AI assistant. It is
intended for preliminary documentation of symptoms and is NOT
a diagnostic tool.

Please share this report with a qualified healthcare provider.
If you are in acute distress, seek emergency care immediately.
${separator}
`.trim();

    const blob = new Blob([content], { type: 'text/plain' });
    const link = document.createElement('a');
    link.download = `MediFlow_Report_${reportData.patientName.replace(/\s+/g, '_')}.txt`;
