...
const STORAGE_KEY = 'mediflow_assessment_state_v5';

const App = () => {
const [view, setView] = useState('chat'); // 'chat' or 'dashboard'
const [isDataLoaded, setIsDataLoaded] = useState(false);
const [chats, setChats] = useState([
{ id: '1', title: 'Assessment 1', messages: [{ role: 'bot', text: 'Welcome to MediFlow. To begin your clinical assessment, could you please provide your full name and age?' }], vitals: {} }
]);
...
// Update vitals in the active chat object
const updateVitals = (key, value) => {
const newVitals = { ...vitals, [key]: value };
setVitals(newVitals);
setChats(prev => prev.map(chat =>
chat.id === activeChatId ? { ...chat, vitals: newVitals } : chat
));
};
...
setVitals(updatedChats[0].vitals || {});
}
}
};
...
<audio ref={audioRef} onEnded={onAudioEnded} hidden />
      ...
      <aside className={`${sidebarOpen ? 'w-80' : 'w-0'} bg-slate-900 transition-all duration-300 flex flex-col overflow-hidden shrink-0 shadow-2xl z-40`}>
        ...
        <div className="p-4 flex flex-col gap-2">
          <button onClick={() => setView('dashboard')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all ${view === 'dashboard' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:bg-white/5'}`}>
        ...
        <div className="flex-1 overflow-y-auto px-4 space-y-1 pb-6">
          ...
          {chats.map(chat => (
            <div key={chat.id} className="group relative flex items-center">
              <button onClick={() => { setActiveChatId(chat.id); setVitals(chat.vitals || {}); setView('chat'); }} className={`w-full text-left px-4 py-3 rounded-xl transition-all flex items-center gap-3 pr-10 ${activeChatId === chat.id && view === 'chat' ? 'bg-white/10 text-white' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}>
      ...
      <div className="flex-1 flex flex-col min-w-0 relative">
        ...
        <header className="bg-white border-b px-8 py-5 flex items-center justify-between shadow-sm shrink-0 z-30">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-lg font-bold text-slate-800">{view === 'dashboard' ? 'Health Dashboard' : activeChat.title}</h1>
          ...
          <div className="flex-1 overflow-y-auto p-10 bg-slate-50 space-y-10 animate-in fade-in slide-in-from-right-4 duration-500">
             <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100">
                   <div className="bg-blue-50 text-blue-600 w-12 h-12 rounded-2xl flex items-center justify-center mb-4"><History size={24} /></div>
                   <p className="text-slate-400 text-xs font-black uppercase tracking-widest">Total Assessments</p>
                   <p className="text-4xl font-black text-slate-900 mt-1">{chats.length}</p>
                </div>
                <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100">
                   <div className="bg-green-50 text-green-600 w-12 h-12 rounded-2xl flex items-center justify-center mb-4"><TrendingUp size={24} /></div>
                   <p className="text-slate-400 text-xs font-black uppercase tracking-widest">Last Update</p>
                   <p className="text-xl font-bold text-slate-900 mt-3">{new Date().toLocaleDateString()}</p>
                </div>
                <div className="bg-slate-900 p-8 rounded-[2rem] shadow-xl text-white">
                   <div className="bg-white/10 text-white w-12 h-12 rounded-2xl flex items-center justify-center mb-4"><Zap size={24} /></div>
                   <p className="text-blue-400 text-xs font-black uppercase tracking-widest">AI Status</p>
                   <p className="text-xl font-bold mt-3 text-white">Clinical Model Active</p>
                </div>
             </div>

             <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
                   <h3 className="text-xl font-black tracking-tight mb-6 flex items-center gap-3"><ClipboardList className="text-blue-600" /> Recent Assessment Summary</h3>
                   <div className="space-y-4">
                      {chats.slice(-3).reverse().map(c => (
                        <div key={c.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl hover:bg-slate-100 transition-colors cursor-pointer" onClick={() => { setActiveChatId(c.id); setVitals(c.vitals || {}); setView('chat'); }}>
                           <div className="flex items-center gap-4">
                              <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-blue-600 shadow-sm font-bold border border-slate-100">#{chats.indexOf(c) + 1}</div>
                              <div>
                                 <p className="font-bold text-slate-900">{c.title}</p>
                                 <p className="text-xs text-slate-400">{c.messages.length} messages</p>
                              </div>
                           </div>
                ...
                <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
                   <h3 className="text-xl font-black tracking-tight mb-6 flex items-center gap-3"><HeartPulse className="text-red-500" /> Baseline Vitals Tracker</h3>
                   <div className="grid grid-cols-3 gap-4">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block ml-2">Temp (°C)</label>
                        <div className="relative group">
                          <Thermometer className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-blue-500 transition-colors" size={16} />
                          <input value={vitals.temp} onChange={(e) => updateVitals('temp', e.target.value)} type="text" placeholder="36.5" className="w-full pl-10 pr-4 py-4 bg-slate-50 border-2 border-transparent focus:border-blue-500 focus:bg-white rounded-2xl transition-all outline-none text-lg font-bold" />
                        </div>
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block ml-2">BP (mmHg)</label>
                        <div className="relative group">
                          <Gauge className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-blue-500 transition-colors" size={16} />
                          <input value={vitals.bp} onChange={(e) => updateVitals('bp', e.target.value)} type="text" placeholder="120/80" className="w-full pl-10 pr-4 py-4 bg-slate-50 border-2 border-transparent focus:border-blue-500 focus:bg-white rounded-2xl transition-all outline-none text-lg font-bold" />
                        </div>
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block ml-2">Pulse (BPM)</label>
                        <div className="relative group">
                          <Activity className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-blue-500 transition-colors" size={16} />
                          <input value={vitals.hr} onChange={(e) => updateVitals('hr', e.target.value)} type="text" placeholder="72" className="w-full pl-10 pr-4 py-4 bg-slate-50 border-2 border-transparent focus:border-blue-500 focus:bg-white rounded-2xl transition-all outline-none text-lg font-bold" />
                        </div>
                      </div>
                   </div>
                   <p className="mt-6 text-xs text-slate-400 italic bg-slate-50 p-4 rounded-xl border border-slate-100">These vitals will be automatically linked to your active assessment for AI analysis.</p>
                </div>
             </div>
          </div>
        ) : (
          ...
          <div className="absolute inset-0 z-50 bg-slate-900/40 backdrop-blur-md flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-2xl h-[90vh] rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
              <div className="px-10 py-8 border-b bg-slate-50 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-black text-blue-600 uppercase tracking-[0.2em] mt-1">Intelligent Assessment Report</p>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-12 bg-white" id="clinical-report-container" ref={reportRef}>
                {generatingReport ? (
                  <div className="h-full flex flex-col items-center justify-center gap-6"><div className="w-16 h-16 border-4 border-blue-100 border-t-blue-600 rounded-full animate-spin"></div><p className="text-sm font-bold text-slate-400 uppercase tracking-widest">Compiling Medical Intelligence...</p></div>
                ) : reportData && (
                  <div className="max-w-prose mx-auto space-y-12 pb-10">
                    ...
                    <div className="pt-10 border-t border-slate-100 space-y-4">
                       <p className="text-[10px] text-slate-400 italic text-center">MediFlow Case Reference: MF-{Math.random().toString(36).substr(2, 9).toUpperCase()}</p>
                       <div className="bg-amber-50 p-6 rounded-3xl border border-amber-100 text-[11px] text-amber-800 leading-relaxed font-medium">
                          <strong>DISCLAIMER:</strong> This is an automated summary intended for recording health data. It does not replace a physical examination or official diagnosis. If symptoms persist or worsen, consult a doctor immediately.
                       </div>
                    </div>
                  </div>
                )}
