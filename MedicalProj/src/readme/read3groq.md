...
contents = [{ role: "user", parts: [{ text: contentsOrPrompt }] }];
} else {
...
setAiAnalysis(null);
    ...
    if (activeTab === "oncology" && imagePreview) {
      ...
      setTimeout(() => {
        ...
        setHistory((prev) => ({
          ...prev,
          [activeTab]: { data: formData[activeTab], prediction: newPrediction },
        }));
      }, 1500);
    }
...
setIsTyping(true);
    ...
    try {
      ...
      const response = await callGemini(
        contents,
        "You are VitaCore AI Agent, a helpful assistant for medical queries. Be professional, empathetic, and provide accurate information."
      );
      setMessages([...newMessages, { role: "assistant", content: response }]);
    } catch (err) {
      setErrorMessage("AI Agent response failed.");
    } finally {
      setIsTyping(false);
    }
...
<div className={`p-3 ${tab.bg} rounded-2xl shadow-md`}>
<tab.icon className={tab.color} size={24} />
</div>
<h3 className="text-2xl font-black">{tab.label}</h3>
</div>
<div className="space-y-4 mb-8">
{Object.entries(val.data).map(([f, v]) => (
<p key={f} className="text-slate-700">
<span className="font-bold capitalize">{f}:</span> {v}
</p>
))}
{val.imagePreview && (
<img
...
Status: {val.prediction.status.toUpperCase()}
</p>
</div>
)}
</div>
...
<p className="col-span-full text-center text-slate-500 font-medium text-lg py-20">
No diagnostic history yet. Perform analyses in other tabs to view
data here.
</p>
)}
</div>
...
const renderAiAgent = () => {
return (
<div className="bg-white rounded-[3.5rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.06)] border border-slate-100 overflow-hidden h-[600px] flex flex-col">
<div className="flex-1 overflow-y-auto p-10 md:p-14 space-y-4">
{messages.map((msg, i) => (
<div
key={i}
className={`flex ${
...
<div
className={`max-w-[70%] p-5 rounded-3xl shadow-md ${
                  msg.role === "user"
...
<div className="p-5 bg-slate-100 rounded-3xl shadow-md flex items-center gap-2">
...
<input
className="flex-1 p-4 border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-purple-500 bg-slate-50/50"
value={chatInput}
...
<button
            onClick={handleSend}
            className="px-8 py-4 bg-purple-600 text-white rounded-2xl font-bold hover:bg-purple-700 transition-all shadow-lg"
          >
Send
</button>
</div>
      ...
      <main className="flex-1 p-6 md:p-14 overflow-y-auto bg-[radial-gradient(circle_at_top_right,_#f1f5f9_0%,_transparent_50%)]">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-12 flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            {["heart", "diabetes", "lung", "oncology"].includes(activeTab) && (
              <button
          ...
          {/* Error Message */}
          {errorMessage && (
            <div className="mb-10 p-6 bg-rose-50 border border-rose-200 rounded-2xl text-rose-700 flex items-center gap-3">
              <AlertCircle size={20} />
              {errorMessage}
            </div>
          )}
          ...
          {activeTab === "dashboard" ? (
            ...
            // Core App Card for diagnostic tabs
