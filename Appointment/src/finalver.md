...
let active = true;
    ...
    class Particle {
      ...
      update() {
        ...
        if (!active) {
          this.opacity -= 0.005; // Fade out slowly when no longer active
        }

        // Only reset to top if celebration is still "active"
    ...
    // Deactivate "spawning" after 3 seconds so they fall off and fade
